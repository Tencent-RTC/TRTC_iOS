...
extension RoomPkViewController{
    ...
    private func bindInteraction(){
           ...
           connectOtherRoomButton.addTarget(self, action: #selector(clickConnectOtherRoomButton), for: .touchUpInside)
     }
    ...
    @objc private func clickConnectOtherRoomButton(){
        ...
        if connectOtherRoomButton.isSelected && checkPkRoomAndUserIdIsValid(){
            let jsonDict = ["roomId":(otherRoomIdTextField.text! as NSString).integerValue,"userId":otherUserIdTextField.text ?? ""] as [String : Any]
